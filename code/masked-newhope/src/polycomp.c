...
typedef struct u32Masked {uint32_t shares[NEWHOPE_MASKING_ORDER+1];} u32Masked;
...
static void sec_mult(Masked* a, Masked* b, Masked* c, unsigned q){
  uint16_t r[2], t;
  int pool_size = (NEWHOPE_MASKING_ORDER*(NEWHOPE_MASKING_ORDER+1))/2;
  ...
  for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) c->shares[i] = (a->shares[i]*b->shares[i])%q;
...
static void range_compare(Masked* x, Masked* z, unsigned c, unsigned k, unsigned q){
  ...
  z->shares[0] = 1;
  for(int i=1; i < NEWHOPE_MASKING_ORDER+1; ++i) z->shares[i] = 0;
...
void convert_A2B_CGV14_32bits(u32Masked* x, u32Masked* y, unsigned k){
  ConvertAB(x->shares, y->shares, k, NEWHOPE_MASKING_ORDER+1);
}
...
int zero_test_mod_mult(Masked* a, int q){
  ...
  for(int j = 0; j < NEWHOPE_MASKING_ORDER+1; ++j){
    ...
    for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) a->shares[i] = (u_j*a->shares[i])%q;
    linear_arithmetic_refresh(a, q);

  }
  B = 0;
  for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) B = (B + a->shares[i])%q;
  if (B == 0) return 1;
  else        return 0;
}
...
int zero_testing_prime_multi(Masked* ppoly, int q, const int SIZE){
  Masked a, t, C;
  for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) fill_masked_mod_q(&a);
  sec_mult(&a, &ppoly[0], &C, q);

  for(int i=1; i < SIZE; ++i){
    for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) fill_masked_mod_q(&a);
    sec_mult(&a, &ppoly[i], &t, q);
    for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) C.shares[j] = (C.shares[j] + t.shares[j])%q;
  }
...
int zero_test_poly_mul_with_reduction(Masked* ppoly, int q, int kappa, const int SIZE){
  ...
  for(int k=0; k < kappa; ++k){
    for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) y[k].shares[i] = 0;
    for(int j=0; j < SIZE; ++j){
      a = uniform_rand16(q);
      for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) y[k].shares[i] = (y[k].shares[i] + a*ppoly[j].shares[i])%q; 
    }
  }
...
void boolean_zero_test(Masked* x, Masked* y, int k, int logk){
  ...
  for(int i=1; i < NEWHOPE_MASKING_ORDER+1; ++i) y->shares[i] = x->shares[i];

  for(int i=0; i < logk; ++i){
    for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) z.shares[j] = y->shares[j] >> (1 << i);
    boolean_refresh(&z, k);
    sec_and(y, &z, y, k); 
  }
  for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) y->shares[i] = (y->shares[i])&1;
...
void bool_poly_zero_test(Masked* ppoly, Masked* b, int k, int logk, const int SIZE){
  ...
  for(int i=1; i < NEWHOPE_MASKING_ORDER+1; ++i) temp1.shares[i] = 0;
...
int newhope_poly_comp_hybrid(Masked* mmasked_poly, uint16_t* ppoly)
{
  ...
  Masked z[l1+1];
  Masked b2;
  unsigned d1 = 8, d2 = 3, q = NEWHOPE_Q;
  unsigned alpha = 17; /* for max 10 shares, alpha = (int) ceil(log2(NEWHOPE_Q*(NEWHOPE_MASKING_ORDER+1)))*/
  int kappa = 10; /* kappa = ceil(128 / log2(Q)) */
  ...
  for(int i=1; i < NEWHOPE_MASKING_ORDER+1; ++i) b2.shares[i] &=1;
  convert_B2A(&b2, z+l1);
