...
{
  if (l == 4){
    uint16_t T[NEWHOPE_MASKING_ORDER+1];
    uint16_t r;
    T[0] = 0x0FF0;

    for(int i=1; i < NEWHOPE_MASKING_ORDER+1; ++i) T[i] = 0;
    
    for(int i=0; i < NEWHOPE_MASKING_ORDER; ++i){
      for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) T[j] = (T[j] << (16-x->shares[i])) + (T[j]>>(x->shares[i]));
      for(int j=0; j < NEWHOPE_MASKING_ORDER; ++j){
        r = rand16();
        T[j] ^= r;
        T[NEWHOPE_MASKING_ORDER] ^= r;
      }
    }
    for(int i=0; i < NEWHOPE_MASKING_ORDER+1; ++i) b->shares[i] = (T[i]>>(x->shares[NEWHOPE_MASKING_ORDER]))&1; 
    for(int j=0; j < NEWHOPE_MASKING_ORDER; ++j){
      r = rand16();
      b->shares[j] ^= r;
    ...
    uint32_t T[NEWHOPE_MASKING_ORDER+1];
    uint32_t r;
    ...
    for(int i=0; i < NEWHOPE_MASKING_ORDER; ++i){
      for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) T[j] = (T[j] << (32-x->shares[i])) + (T[j]>>(x->shares[i]));
      for(int j=0; j < NEWHOPE_MASKING_ORDER; ++j){
        r = rand32();
        T[j] ^= r;
        T[NEWHOPE_MASKING_ORDER] ^= r;
    ...
    uint64_t T[NEWHOPE_MASKING_ORDER+1];
    uint64_t r;
    ...
    for(int i=0; i < NEWHOPE_MASKING_ORDER; ++i){
      for(int j=0; j < NEWHOPE_MASKING_ORDER+1; ++j) T[j] = (T[j] << (64-x->shares[i])) + (T[j]>>(x->shares[i]));
      for(int j=0; j < NEWHOPE_MASKING_ORDER; ++j){
        r = rand64();
        T[j] ^= r;
        T[NEWHOPE_MASKING_ORDER] ^= r;
    ...
    uint64_t T[4][NEWHOPE_MASKING_ORDER+1];
    uint64_t T_p[4][NEWHOPE_MASKING_ORDER+1];
    uint64_t r;
    ...
    for(int i=0; i < NEWHOPE_MASKING_ORDER; ++i){
      ...
      for(int j=0; j < NEWHOPE_MASKING_ORDER; ++j){
        r = rand64();
        T[0][j] ^= r;
        T[0][NEWHOPE_MASKING_ORDER] ^= r;
        r = rand64();
        T[1][j] ^= r;
        T[1][NEWHOPE_MASKING_ORDER] ^= r;
        r = rand64();
        T[2][j] ^= r;
        T[2][NEWHOPE_MASKING_ORDER] ^= r;
        r = rand64();
        T[3][j] ^= r;
        T[3][NEWHOPE_MASKING_ORDER] ^= r;
